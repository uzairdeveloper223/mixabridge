...
6. **Configure Loops & Root Motion** — check the boxes inline to instantly loop animations or strip translation from the root bone (in-place)
7. **Rename** — double-click any animation in the list to rename it before processing
8. **Remove** — select an animation and click "Remove Selected" to drop it from the queue
9. **Pick a library** — choose an existing AnimationLibrary on the player, or create a new one with a custom name
10. **Process All** — the library gets built and attached to your AnimationPlayer
11. **Edit Existing** — modifying checkboxes on existing animations instantly saves the `.tres` library, no re-processing needed
